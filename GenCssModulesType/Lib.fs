...
let removeDot (rule: string) = rule.Substring(1)

let typeDefOfRule rule = "export const " + rule + ": string;"

let toTypeDefFile content typeDef = content + typeDef + "\n"
    ...
    let rec handler retries =
        if retries > 10_000 then
            ...
            try
                ...
                |> parseRules
                |> List.map (removeDot >> typeDefOfRule)
                |> function
